...
class Handler(SimpleHTTPRequestHandler):
    def translate_path(self,path):
        ...
        if name=='poster-admin.html': return str(folder/'poster-admin.html')
        if name=='event-editor.html': return str(folder/'event-editor.html')
    ...
    def end_headers(self):
        ...
        self.send_header('Content-Security-Policy',"default-src 'self'; script-src 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' blob:; connect-src 'none'; form-action 'none'; object-src 'none'; base-uri 'none'")
        super().end_headers()
