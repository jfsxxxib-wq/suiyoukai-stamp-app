...
const allowedResultKeys = new Set([
  'sentinel', 'cf_internal_tables', 'sqlite_internal_tables', 'schema_meta_tables',
  'schema_meta_rows', 'd1_migrations_tables', 'goencho_tables', 'unexpected_user_tables',
  'unexpected_user_objects', 'named_indexes', 'unexpected_named_indexes', 'foreign_keys',
  'business_rows', 'schema_version', 'total_columns', 'added_columns', 'new_indexes',
  'column_shape_matches', 'new_index_shape_matches', 'new_index_column_matches',
  'new_index_predicate_matches',
]);
...
export function assertFixedEndpoint(candidate) {
  let parsed;
  try { parsed = new URL(candidate); } catch { throw new SafeTransportError('ENDPOINT_GUARD'); }
  const expectedPath = `/client/v4/accounts/${FIXED_ACCOUNT_ID}/d1/database/${FIXED_DATABASE_UUID}/query`;
  if (candidate !== FIXED_ENDPOINT || parsed.protocol !== 'https:' || parsed.hostname !== 'api.cloudflare.com' || parsed.port !== '' || parsed.pathname !== expectedPath || parsed.search !== '' || parsed.hash !== '') {
...
function normalizeResponse(value) {
  ...
  const normalized = { success: value.success === true, result: [] };
      ...
      safeItem.results = item.results.map((row) => {
        ...
        if (row && typeof row === 'object') {
          for (const [key, field] of Object.entries(row)) {
            if (allowedResultKeys.has(key) && ['string', 'number', 'boolean'].includes(typeof field)) safeRow[key] = field;
          }
        }
        return safeRow;
      });
...
export function createLiveTransport({ fetchLike, timeoutMs = 30_000, setTimer = setTimeout, clearTimer = clearTimeout }) {
  if (typeof fetchLike !== 'function' || !Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 30_000) throw new SafeTransportError('LOCAL_PROCESS_OR_CONFIG');
  const debug = { active_requests: 0, client_attempts: 0, raw_response_held: false, raw_json_held: false };
  ...
  async function send({ method = 'POST', endpoint = FIXED_ENDPOINT, tokenBuffer, body }) {
    ...
    if (!Buffer.isBuffer(tokenBuffer) || tokenBuffer.length < 1) throw new SafeTransportError('AUTH_OR_SCOPE');
    if (body === null || typeof body !== 'object' || Array.isArray(body)) throw new SafeTransportError('LOCAL_PROCESS_OR_CONFIG');
    ...
    try {
      rawResponse = await fetchLike(FIXED_ENDPOINT, {
        ...
        headers: { 'Content-Type': 'application/json', Authorization: authorization }, body: requestBody,
      });
      debug.raw_response_held = true;
      if (rawResponse?.redirected === true || rawResponse?.url !== FIXED_ENDPOINT) throw new SafeTransportError('ENDPOINT_GUARD', Number.isInteger(rawResponse?.status) ? rawResponse.status : null, 1);
      ...
      try { rawJson = await rawResponse.json(); } catch { throw new SafeTransportError('REMOTE_RESULT_INVALID', rawResponse.status, 1); }
      debug.raw_json_held = true;
      return { response: normalizeResponse(rawJson), safe_meta: { http_status: rawResponse.status, client_attempts: 1 } };
    } catch (error) {
  ...
  return { send, getDebugState() { return { ...debug }; } };
}
