...
export function createTokenReceiver({ onToken, maxBytes = 4096, closeAfterAccept = true }) {
  ...
  const state = { accepted: false, token_buffer_held: false, token_buffer_zeroed: false, server_closed: false };
  let server;

  async function handle(req, res) {
    if (!isLoopbackAddress(req.socket.remoteAddress)) return send(res, 403, '<h1>拒否しました</h1>');
    if (req.method === 'GET' && req.url === '/') {
      return send(res, state.accepted ? 410 : 200, state.accepted ? '<h1>使用済みです</h1>' : formHtml);
    }
    if (req.method !== 'POST' || req.url !== '/submit' || state.accepted) {
      return send(res, state.accepted ? 410 : 404, '<h1>受理できません</h1>');
    }
    const declaredLength = Number(req.headers['content-length'] ?? 0);
    if (!Number.isFinite(declaredLength) || declaredLength < 1 || declaredLength > maxBytes) {
      return send(res, 413, '<h1>入力サイズを受理できません</h1>');
    }
    ...
    req.on('end', async () => {
      ...
      if (!tokenText) {
        tokenText = null;
        return send(res, 400, '<h1>Tokenが空です</h1>');
      }
      ...
      try {
        ...
        if (closeAfterAccept) server.close(() => { state.server_closed = true; });
      }
    });
  }

  server = http.createServer((req, res) => { handle(req, res).catch(() => send(res, 500, '<p>STOPPED_SAFE</p>')); });
  return {
    async listen() {
