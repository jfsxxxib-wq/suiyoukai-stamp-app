import { runMigrationSafely } from './rest-migrate-core.mjs';
...
function safeSummary(result, debug) {
  return {
    status: result?.status ?? 'STOPPED', phase: result?.phase ?? 'UNKNOWN',
    recovery_timestamp_rfc3339: result?.recovery_timestamp_rfc3339 ?? null,
    recovery_timestamp_unix: result?.recovery_timestamp_unix ?? null,
    ...
    migration_requests: Number.isInteger(result?.migration_requests) ? result.migration_requests : 0,
    result_counts: result?.result_counts ?? null, safe_failure: result?.safe_failure ?? null,
    retry_performed: result?.retry_performed === true, restore_performed: result?.restore_performed === true,
    d1_migrations_touched: result?.d1_migrations_touched === true, token_stored: result?.token_stored === true,
...
const receiver = createTokenReceiver({
  ...
  async onToken(tokenBuffer) {
    const transport = async ({ method, path, token, body }) => {
      ...
      return live.send({ method, tokenBuffer, body });
    };
    const result = await runMigrationSafely({ transport, token: tokenBuffer, now: new Date() });
    ...
    if (result?.status === 'PASS' && result?.client_attempts !== debug.client_attempts) {
      finalSummary = safeSummary({ ...result, status: 'STOPPED', phase: 'LIVE_CLIENT_ATTEMPTS_CHECK', safe_failure: {
        phase: 'LIVE_CLIENT_ATTEMPTS_CHECK', classification: 'REMOTE_ATTEMPTS_INVALID', http_status: null,
        cloudflare_numeric_codes: [], client_attempts: debug.client_attempts, total_attempts: null,
...
const shutdown = async () => { await receiver.close().catch(() => {}); process.exit(finalSummary?.status === 'PASS' ? 0 : 1); };
process.once('SIGINT', shutdown);
process.once('SIGTERM', shutdown);
while (!receiver.getState().server_closed) await new Promise((resolvePromise) => setTimeout(resolvePromise, 100));
