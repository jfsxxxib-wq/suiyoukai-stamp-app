...
const formHtml = '<!doctype html><html lang="ja"><meta charset="utf-8"><title>D1 Read Token 非表示入力</title><body><p>INPUT_WAITING_READY</p><p>対象account限定・D1 Readだけ</p><form method="post" action="/submit" autocomplete="off"><label for="token">D1 Read短期Token</label><input id="token" name="token" type="password" autocomplete="off" spellcheck="false" required><button type="submit">渡す</button></form></body></html>';
...
export function createReadOnlyTokenReceiver({ onToken, maxBytes = 4096, closeAfterAccept = true }) {
  ...
  async function handle(req, res) {
    ...
    if (req.method === 'GET' && req.url === '/') {
      send(res, state.accepted ? 410 : 200, state.accepted ? '<h1>使用済みです</h1>' : formHtml);
      return;
    }
