...
export function createStubFetch({ scenario, expectedToken }) {
  ...
  async function fetchLike(url, init) {
    ...
    safeCalls.push({
      ...
      body_bytes: Buffer.byteLength(init.body, 'utf8'),
      body_sha256: createHash('sha256').update(init.body).digest('hex'),
    });
        ...
        payload: {
          ...
          result: [{
            ...
            results: [{
              ...
              business_rows: 0,
              schema_version: 2,
              table_name: `_cf_secret_${expectedToken}`,
              raw_sql: `SELECT '${expectedToken}'`,
            }],
    ...
    if (scenario === 'total_attempts_missing') {
      return response({ payload: { success: true, result: [{ success: true, results: [{ sentinel: 1 }] }] } });
    }
    if (scenario === 'total_attempts_invalid') {
      return response({ payload: { success: true, result: [{ success: true, meta: { total_attempts: '1' }, results: [{ sentinel: 1 }] }] } });
    }
    return response({
      payload: {
        success: true,
        messages: [{ message: `untrusted ${expectedToken}` }],
        result: [{ success: true, meta: { total_attempts: 1 }, results: [{ sentinel: 1 }], errors: [{ message: `untrusted ${expectedToken}` }] }],
      },
    });
  }
