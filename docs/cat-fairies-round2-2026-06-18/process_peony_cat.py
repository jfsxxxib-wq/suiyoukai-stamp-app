...
trans=base/f'{name}-transparent-candidate.png'
canvas.save(trans)
...
for yy in range(0,canvas.height,s):
    for xx in range(0,canvas.width,s):
        col=(235,235,235,255) if ((xx//s+yy//s)%2)==0 else (255,255,255,255)
        d.rectangle([xx,yy,xx+s-1,yy+s-1],fill=col)
checker.alpha_composite(canvas)
checker.save(base/f'{name}-transparent-checker-preview.png')
...
d.text((40,25),'Peony white longhair cat fairy original',fill=(80,70,55))
d.text((560,25),'Transparent candidate',fill=(80,70,55))
