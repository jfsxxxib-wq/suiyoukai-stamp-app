...
name='peony-white-longhair-cat-fairy'
img=Image.open(base/f'{name}-original.png').convert('RGBA')
w,h=img.size; pix=img.load()
# use only far corners for background, and very tight threshold to avoid white fur
...
trans=base/f'{name}-transparent-candidate-v2.png'; canvas.save(trans)
...
d=ImageDraw.Draw(sheet); d.text((40,25),'Peony original',fill=(80,70,55)); d.text((560,25),'Transparent candidate v2',fill=(80,70,55))
sheet.save(base/f'{name}-background-check-v2-2026-06-18.png')
print(trans)
print(base/f'{name}-background-check-v2-2026-06-18.png')
