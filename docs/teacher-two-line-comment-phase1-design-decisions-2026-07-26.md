...
- `ENVIRONMENT_STATUS`は、試験操作を許可する`ACTIVE`または保管状態の`ARCHIVED`だけを使用する。
...
`ENVIRONMENT_STATUS`が`ACTIVE`以外、不正、未設定、取得不能の場合は、スプレッドシートを開かず、現在値と履歴を変更せず、安全な`temporary_error`を返して処理を停止する。その他の検査も、一つでも不一致、未設定、取得不能がある場合は、スプレッドシートの読み書きを行わず処理を停止する。誤った接続先へ試し書きをして確認する方式にはしない。
...
> 第1段階のApps Scriptは、試験用スプレッドシートへ紐づけたコンテナバインド形式ではなく、Google Drive上で独立して管理できるスタンドアロン形式として作成する。スプレッドシートとApps Scriptは、それぞれ別のファイルID、名称、保管・削除承認を持つ。
>
> 保管状態への移行時は、Script Propertiesの`ENVIRONMENT_STATUS`を`ACTIVE`から`ARCHIVED`へ変更する。すべてのAPI処理はスプレッドシートへのアクセス前にこの値を検査し、`ACTIVE`以外では処理を停止する。名称変更は第二の安全確認として使用し、名称だけを唯一の停止手段としない。
>
...
6. Script Propertiesの`ENVIRONMENT_STATUS`を`ACTIVE`から`ARCHIVED`へ変更する
...
| `ARCHIVED` | 試験終了後の保管状態 | スプレッドシートを開かず処理停止 |

- 大文字・小文字を含め完全一致で検査する。
- 未設定、空文字、その他の値は`ACTIVE`として扱わない。
- すべてのPOSTとGETで、スプレッドシートを開く前に検査する。
- `ACTIVE`以外では現在値と履歴を変更しない。
- `ACTIVE`以外では安全な`temporary_error`を返し、内部設定値やファイルIDを返さない。
- `ARCHIVED`を`ACTIVE`へ戻す操作は、悦子さんの別途承認なしに行わない。
...
- 名称だけを元に戻しても試験を再開しない。
- 再開が必要な場合は、対象、理由、再開期間、終了後の再保管手順について悦子さんの別途承認を得る。
